...
async function init(){
  console.log('[Squad] Init started');
  try {
...
async function loadSquad(){
  try {
    ...
    console.error('[Squad] loadSquad error:', e);
...
async function loadPointsByPlayer(){
  try {
    ...
    renderSquad();
  } catch(e) { console.warn('[Squad] loadPointsByPlayer error:', e); }
}
...
function setFilter(role){
  ...
  for(var i=0; i<tabs.length; i++) {
    tabs[i].classList.toggle('active', order[i] === role);
  }
  renderSquad();
}
...
function renderSquad(){
  ...
  if(!list.length){
    grid.innerHTML = '<div class="empty-state" style="grid-column:1/-1"><div class="empty-state-icon">!</div><div class="empty-state-text">' + (_squad.length ? 'No players category' : 'Squad empty') + '</div></div>';
    return;
  }
  ...
  for(var i=0; i<list.length; i++){
    ...
    var code = (p.ipl_team||'').replace(/\s+/g,'').toUpperCase();
    var color = C[code] || '#f0b429';
    ...
    var cls = 'player-card' + (isCap ? ' captain' : isVC ? ' vice-cap' : '');
    html += '<div class="' + cls + '" style="--ipl-color:' + color + '; animation-delay:' + (i*0.05) + 's">' +
...
function renderSummary(){
  ...
  var el = document.getElementById('squad-summary'); 
  if(!el) return;
  ...
  var res = '<div style="display:flex; gap:20px; flex-wrap:wrap; align-items:center;">';
  for(var i=0; i<items.length; i++){
    var it = items[i];
    var sep = i ? '<div style="width:1px; height:32px; background:var(--border); flex-shrink:0;"></div>' : '';
    var vh = it.plain ? '<div style="font-family:var(--f-display); font-weight:700; font-size:15px;">' + UI.esc(String(it.val)) + '</div>' :
    ...
    res += sep + '<div><div style="font-size:10px; text-transform:uppercase; letter-spacing:1px; color:var(--text3); font-family:var(--f-ui); font-weight:700; margin-bottom:2px;">' + it.lbl + '</div>' + vh + '</div>';
  }
  el.innerHTML = res + '</div>';
}
...
function renderRoleBreakdown(){
  ...
  var el = document.getElementById('role-bars'); 
  if(!el) return;
  
  var roles = ['Batter', 'Bowler', 'All-Rounder', 'Wicket-Keeper'];
  var html = '';
  for(var j=0; j<roles.length; j++){
    var r = roles[j], n = counts[r], min = mins[r], met = n >= min;
    html += '<div style="flex:1; min-width:100px;">' +
      '<div style="display:flex; justify-content:space-between; font-size:12px; margin-bottom:5px;">' +
        '<span style="font-family:var(--f-ui); font-weight:700; color:var(--text2);">' + labels[r] + '</span>' +
      ...
      '<div style="height:5px; background:var(--bg3); border-radius:3px; overflow:hidden;">' +
        '<div style="height:100%; background:' + (met ? colors[r] : 'var(--red)') + '; border-radius:3px; width:' + Math.min(100, (n/4)*100) + '%; transition:width 1s ease;"></div>' +
      '</div></div>';
  }
  el.innerHTML = html;
}
...
async function exportSquadPDF(){
  try {
    ...
    for(var k=0; k<list.length; k++){
      var sqP = list[k], p = sqP.player || {};
      var tags = (p.is_overseas ? 'OS' : '');
      ...
      rows += '<tr><td>' + (k+1) + '</td>' +
        ...
        '<td style="text-align:center; font-weight:bold; color:#b7791f">' + (cap ? 'Captain' : vc ? 'Vice' : '') + '</td></tr>';
    }
    
    var tName = (_myTeam && _myTeam.team_name) || 'My Squad';
    var dStr = new Date().toLocaleString('en-IN');
    
    var parts = [
      '<!DOCTYPE html><html><head><meta charset="UTF-8"><title>', UI.esc(tName), '</title>',
      ...
      '<h1>', UI.esc(tName), '</h1>',
      '<p style="font-size:12px;color:#666">BFL IPL 2026 . ', UI.esc(dStr), '</p>',
      '<p>Players: ', list.length, '/12 OS: ', os, '/4</p>',
      '<table><thead><tr><th>#</th><th>Player</th><th>Role</th><th>IPL</th><th>Status</th></tr></thead><tbody>', rows, '</tbody></table>',
      '</body></html>'
    ];
    ...
    UI.toast('PDF opened - Ctrl+P to print', 'success', 5000);
