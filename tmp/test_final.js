...
async function init(){
  ...
  try {
    ...
    if(!sess) { console.log('[Squad] No session'); return; }
...
async function loadSquad(){
  try {
    console.log('[Squad] Loading squad for:', _myTeamId);
    var raw = await API.fetchSquad(_myTeamId);
    _squad = safeArr(raw);
    console.log('[Squad] Found counts:', _squad.length);
    renderSquad(); 
    renderSummary(); 
    renderRoleBreakdown();
  } catch(e) {
...
function renderSquad(){
  ...
  grid.innerHTML = list.map(function(sp, i){
    ...
    var cardCls = 'player-card' + (isCap ? ' captain' : isVC ? ' vice-cap' : '');
...
<body>
  ...
  <p style="font-size:12px; color:#666">BFL IPL 2026 . ${UI.esc(dateStr)}</p>
  <p>Players: ${list.length}/12 &middot; OS: ${os}/4</p>
  <table>
    <thead><tr><th>#</th><th>Player</th><th>Role</th><th>IPL</th><th>Status</th></tr></thead>
    ...
    UI.toast('PDF opened - press Ctrl+P to print', 'success', 5000);
  } catch(e) {
    console.error('[PDF]', e);
